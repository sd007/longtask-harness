...
批准时，宿主支持交互控件就点击“批准并执行”或“修改方案”。在 CLI、手机端或无按钮环境中，直接用自然语言表达决定即可，不需要输入固定句式。
...
Gate 通过后，宿主支持交互控件就显示“接受交付”或“需要修改”；无按钮时用自然语言表达即可。只有你明确接受后，Codex 才能内部执行 `accept`。如果拒绝，请给出可观察的失败或缺口，目标会回到执行态继续迭代。
