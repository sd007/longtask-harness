...
提交说明遵循当前对话语言，仓库既有规范优先；控制器不强制英文。方案批准前必须显示决策检查，只有高影响未决项才会触发用户询问。宿主提供按钮时，方案使用“批准并执行/修改方案”、交付使用“接受交付/需要修改”；无按钮时降级为自然语言。按钮或自然语言只转译为现有状态转换，不能绕过 `plan-check`、验收 Gate 或证据新鲜度。
