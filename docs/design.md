...
v0.11 的 `state.json` 使用 schema v3，是唯一当前事实源。v1/v2 明确不兼容，读取时只报错并提示归档后重新初始化，不自动迁移或删除。外部展示区分存储状态和有效状态：证据失效时，存储的 `READY_FOR_ACCEPTANCE` 有效显示为 `VERIFYING`。
...
v0.11 能降低遗漏、漂移、早停、并发写丢失和无证据结论，同时减少普通任务的规划、审批、分支和审计开销；但不能保证验收标准本身正确、外部陈述真实或测试覆盖未知缺陷。它提高可靠性和证据透明度，不宣称绝对完美达成。

## 未来扩展条件

只有观测到实际瓶颈后再扩展：原生跨平台 CI、真实外部服务调用框架和 token/费用平台不进入 v0.11；概率置信度仍需历史基准集和真实结果标签。
